...
//  SimpleARRuler
//
//  Created by Neeraj Chandra on 18/08/17.
...
class ViewController: UIViewController, ARSCNViewDelegate {
    ...
    override func viewDidLoad() {
        ...
        //to show feature points detected on scene surface
    ...
    // Detect the location of a touch
    override func touchesBegan(_ touches: Set<UITouch>, with event: UIEvent?){
        // if there is a third touch remove previous touch nodes
        if dotNodes.count >= 2 {
            for dot in dotNodes{
        ...
        if let touchLocation = touches.first?.location(in: sceneView){
            ...
            // if points are detected on surface by hittest add dot there
    ...
    func updateText(text: String, atPosition position: SCNVector3){
        textNode.removeFromParentNode()//remove previous distance if there is
